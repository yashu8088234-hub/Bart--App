...
st.markdown("<h1 style='text-align: center; font-size: 58px; font-weight: 800; color: #111111; margin-top: 15px;'>Operations management <br><span style='background: linear-gradient(90deg, #2ED47A 0%, #20C997 100%); -webkit-background-clip: text; -webkit-text-fill-color: transparent;'>just got easier.</span></h1>", unsafe_allow_html=True)
...
with grid_right:
    ...
    if time.time() < st.session_state.mgmt_lock_until:
        st.button(f"Console Locked ({int(st.session_state.mgmt_lock_until - time.time())}s) 🔒", disabled=True, use_container_width=True)
    elif st.button("Unlock Admin Panel", use_container_width=True, key="mgmt_btn"):
