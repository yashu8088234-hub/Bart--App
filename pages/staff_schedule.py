...
].copy() if not all_data_df.empty else pd.DataFrame(
    columns=["Branch", "Name", "Role"] + DAYS
)
...
if edit_mode:
    ...
    if st.session_state.deleted_staff:
        df_display = df_display[
            ~df_display["Name"].isin(st.session_state.deleted_staff)
        ].reset_index(drop=True)
...
if st.button("⬅ Back"):
    st.switch_page("pages/staff_dashboard.py")import streamlit as st
import pandas as pd
import gspread
import time
import re
